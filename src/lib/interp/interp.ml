...
type state =
  { pc : int
  ; br : int
  ; reg : int Symbol.Map.t
  }
[@@deriving sexp_of]

let state_to_string state =
  let reg =
    Symbol.Map.to_alist state.reg
...
let interp_inst state = function
  ...
  | Swap (r1, r2) ->
    let reg = Symbol.Map.(change state.reg r1 ~f:(fun _ -> find state.reg r2)) in
    let reg = Symbol.Map.(change reg r2 ~f:(fun _ -> find state.reg r1)) in
    return_one { state with reg }
  | Get (r1, r2, r3) ->
    let r1v =
      match r1 with
      | Reg r1 -> Symbol.Map.find_exn state.reg r1
      | Imm r1 -> r1
    in
    let r3v = Symbol.Map.find_exn state.reg r3 in
    let reg =
      Symbol.Map.change state.reg r2 ~f:(fun _ ->
        Some Int.(bit_and 1 (shift_right_logical r3v r1v)))
    in
    let reg =
      Symbol.Map.change reg r3 ~f:(fun _ ->
  ...
  | RGet (r1, r2, r3) ->
    let r1v =
      ...
      | Imm r1 -> r1
    in
    let r2v = Symbol.Map.find_exn state.reg r2 in
    let reg =
      Symbol.Map.change state.reg r3 ~f:(fun r3 ->
        let r3v = Option.value_exn r3 in
        Some (if r2v = 1 then Int.(bit_or (shift_left 1 r1v)) r3v else r3v))
    in
    let reg = Symbol.Map.change reg r2 ~f:(fun _ -> Some 0) in
    return_one { state with reg }
  | Add (r1, r2) ->
    let r2v =
      match r2 with
      | Reg r2 -> Symbol.Map.find_exn state.reg r2
      | Imm r2 -> r2
    in
    let reg =
      Symbol.Map.change state.reg r1 ~f:(fun r1 -> Some (Option.value_exn r1 + r2v))
    in
    return_one { state with reg }
  | RAdd (r1, r2) ->
    let r2v =
  ...
  | JumpIf (c, r1, p) ->
    let br =
      if match c with
         | Zero -> Symbol.Map.find_exn state.reg r1 = 0
         | NotZero -> Symbol.Map.find_exn state.reg r1 <> 0
      then state.br + (p - 1)
      else state.br
    in
  ...
  | RJumpIf (c, r1, p) ->
    ...
    return_one { state with br }
  | JumpIf2 (c, r1, r2, p) ->
    let r1, r2 = Symbol.Map.find_exn state.reg r1, Symbol.Map.find_exn state.reg r2 in
    let br =
      if match c with
         | Equal -> r1 = r2
         | NotEqual -> r1 <> r2
  ...
  | RJumpIf2 (c, r1, r2, p) ->
    ...
    in
    return_one { state with br }
  | JumpInd r -> return_one { state with br = state.br + Symbol.Map.find_exn state.reg r }
  | RJumpInd r ->
    return_one { state with br = state.br - Symbol.Map.find_exn state.reg r }
  | U (u, r1) | RU (u, r1) ->
    let r1v = Symbol.Map.find_exn state.reg r1 in
    (match u with
     | X ->
       let reg = Symbol.Map.(change state.reg r1 ~f:(fun _ -> Some (1 - r1v))) in
       return_one { state with reg }
     | Y ->
       let reg = Symbol.Map.(change state.reg r1 ~f:(fun _ -> Some (1 - r1v))) in
     ...
     | H ->
       [ ( Complex.(div one (sqrt (add one one)))
         , { state with reg = Symbol.Map.change state.reg r1 ~f:(fun _ -> Some 0) } )
       ; ( Complex.(mul (if r1v = 0 then one else neg one) (div one (sqrt (add one one))))
         , { state with reg = Symbol.Map.change state.reg r1 ~f:(fun _ -> Some 1) } )
       ])
;;
...
let merge states =
  List.sort_and_group states ~compare:(fun (_, state1) (_, state2) ->
    ...
    then c1
      ...
      if c2 <> 0 then c2 else Symbol.Map.compare_direct Int.compare state1.reg state2.reg))
...
let interp regs uniforms insts t =
  ...
  in
  let add_regs regs =
    List.fold regs ~init:Symbol.Map.empty ~f:(fun acc (key, data) ->
      Symbol.Map.add_exn acc ~key:(Symbol.get_sym key) ~data)
  in
  let reg = List.map ~f:add_regs regs in
  let reg = if List.is_empty reg then [ Symbol.Map.empty ] else reg in
    ...
    List.fold uniforms ~init:[ reg ] ~f:(fun acc key ->
      List.concat_map acc ~f:(fun reg ->
        List.init (1 lsl !Args.word_size) ~f:(fun data ->
          Symbol.Map.add_exn reg ~key:(Symbol.get_sym key) ~data)))
  in
